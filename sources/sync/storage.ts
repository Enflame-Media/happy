...
interface StorageState {
    ...
    socketStatus: 'disconnected' | 'connecting' | 'authenticating' | 'connected' | 'error';
    ...
    setSocketStatus: (status: 'disconnected' | 'connecting' | 'authenticating' | 'connected' | 'error') => void;
...
export const storage = create<StorageState>()((set, get) => {
    ...
    return {
        ...
        setSocketStatus: (status: 'disconnected' | 'connecting' | 'authenticating' | 'connected' | 'error') => set((state) => {
            const now = Date.now();
        ...
        updateSessionModelMode: (sessionId: string, mode: 'opus' | 'sonnet' | 'haiku' | 'gpt-5-minimal' | 'gpt-5-low' | 'gpt-5-medium' | 'gpt-5-high' | 'gpt-5-codex-low' | 'gpt-5-codex-medium' | 'gpt-5-codex-high') => set((state) => {
            const session = state.sessions[sessionId];
            if (!session) return state;
        ...
        deleteSession: (sessionId: string) => set((state) => {
            ...
            // HAP-588: Clear cached messages for this session
