...
export const pl: TranslationStructure = {
    ...
    errors: {
        ...
        voiceServiceUnavailable: 'Usługa głosowa jest niedostępna',
    ...
    session: {
        ...
        inputPlaceholderArchived: 'Sesja jest zarchiwizowana',
        // HAP-392: Archived session banner
        archivedBannerText: 'Ta sesja jest zarchiwizowana',
    ...
    server: {
        ...
        invalidJsonResponse: 'Odpowiedź serwera nie jest prawidłowym JSON. Upewnij się, że URL wskazuje na API Happy Server, a nie na stronę internetową.',
        missingRequiredFields: ({ fields }: { fields: string }) => `W odpowiedzi serwera brakuje wymaganych pól: ${fields}`,
        incompatibleVersion: ({ serverVersion, requiredVersion }: { serverVersion: string; requiredVersion: string }) =>
            `Wersja serwera ${serverVersion} nie jest kompatybilna. Minimalna wymagana wersja to ${requiredVersion}.`,
        httpError: ({ status }: { status: number }) => `Serwer zwrócił błąd HTTP ${status}`,
        emptyResponse: 'Serwer zwrócił pustą odpowiedź',
    },
    ...
    components: {
        emptyMainScreen: {
            ...
            welcomeSubtitle: 'Steruj Claude Code z telefonu z szyfrowaniem end-to-end',
            readyToCode: 'Gotowy do kodowania?',
            installCli: 'Zainstaluj Happy CLI',
            runIt: 'Uruchom je',
            scanQrCode: 'Zeskanuj kod QR',
            openCamera: 'Otwórz kamerę',
            scanQrToConnect: 'Zeskanuj QR, aby połączyć',
            featureEncryption: 'Szyfrowanie end-to-end',
            featureRemoteControl: 'Steruj zewsząd',
            featureRealtime: 'Synchronizacja w czasie rzeczywistym',
        },
        emptySessionsTablet: {
            // Used by EmptySessionsTablet component (tablet empty state)
            welcomeTitle: 'Witaj w Happy Coder!',
            welcomeDescription: 'Połącz terminal, aby rozpocząć. Uruchom happy-cli na komputerze i zeskanuj kod QR.',
            noActiveSessions: 'Brak aktywnych sesji',
            startSessionOnMachine: 'Rozpocznij nową sesję na dowolnej z połączonych maszyn.',
            openTerminalToStart: 'Otwórz terminal na komputerze, aby rozpocząć sesję.',
            startNewSession: 'Rozpocznij nową sesję',
            featureEncrypted: 'Zaszyfrowane',
            featureRealtime: 'W czasie rzeczywistym',
        },
        errorBoundary: {
    ...
    message: {
        ...
        showMore: ({ lines }: { lines: number }) => `Pokaż jeszcze ${lines} linii`,
        showLess: 'Pokaż mniej',
    },
    ...
    planLimits: {
        ...
        title: 'Limity użycia planu',
        weeklyLimits: 'Limity tygodniowe',
        learnMore: 'Dowiedz się więcej o limitach użycia',
        used: 'wykorzystano',
        resetsIn: ({ time }: { time: string }) => `Resetuje się za ${time}`,
        resetsAt: ({ time }: { time: string }) => `Resetuje się ${time}`,
        lastUpdated: ({ time }: { time: string }) => `Ostatnia aktualizacja: ${time}`,
        unavailable: 'Limity użycia nie są dostępne dla Twojego obecnego dostawcy',
    ...
    allowedCommands: {
        ...
        sectionTitle: 'Dozwolone Komendy',
        summary: ({ count }: { count: number }) => `${count} komend dostępnych do zdalnego wykonania`,
        restricted: 'ograniczone',
        allArgs: 'wszystkie arg.',
        fetchError: 'Nie udało się załadować dozwolonych komend',
        noCommands: 'Brak dostępnych komend',
        securityNote: 'Komendy nieznajdujące się na liście są blokowane ze względów bezpieczeństwa',
    },
